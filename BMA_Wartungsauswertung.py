...
#!/usr/bin/env python3
"""
BMA-Wartungsauswertung für ODS-, XLSX- und alte XLS-Prüflisten
...
- Checkbox „Minimalwartung“ überschreibt bereits belegte Melder mit dem ausgewählten Quartal
...
Hinweis:
ODS bleibt ODS, XLSX bleibt XLSX. Alte XLS-Vorlagen werden vor der Bearbeitung automatisch in XLSX konvertiert. Die Originalvorlage bleibt unverändert.
"""
...
def feuermelder_aus_esser_pdf_lesen(
    ...
    # Ereignisse werden nach dem vollständigen Ereignisnamen geprüft.
    # Dadurch kann "FEUER-ENDE" niemals versehentlich als "FEUER" zählen.
    erlaubte_ereignisse = {"FEUER"}
    if voralarm_beruecksichtigen:
        erlaubte_ereignisse.update({"VORALARM", "VORALARM-ENDE"})

    row_pattern = re.compile(
        r"^\s*\d+?(\d{2}\.\d{2}\.)"
        r"(\d{1,2}:\d{2}:\d{2})\s*"
        r"(FEUER(?:-ENDE)?|VORALARM(?:-ENDE)?)\s+"
    ...
    for text in page_texts:
        for raw_line in text.splitlines():
            ...
            if not match:
                continue
            event_name = match.group(3).upper()
            if event_name not in erlaubte_ereignisse:
                # Insbesondere FEUER-ENDE wird hier immer verworfen.
                continue
...
def txt_pruefbericht_schreiben(result: Dict[str, object], quarter: int) -> str:
    ...
    lines: List[str] = [
        ...
        f"Eingetragen: {len(result.get('inserted', []))}",
        f"Davon überschrieben (Minimalwartung): {len(result.get('overwritten', []))}",
        f"Bereits belegt und nicht überschrieben: {len(result.get('occupied', []))}",
...
def ods_pruefliste_ausfuellen(
    ...
    try:
        with zipfile.ZipFile(ods_path, "r") as source:
            ...
            overwritten: List[Tuple[int, int, str]] = []
            ...
            for group, detector in sorted(event_detectors):
                ...
                if previous_value:
                    if not minimalwartung:
                        occupied.append((group, detector, previous_value))
                        continue
                    overwritten.append((group, detector, previous_value))
        ...
        for group, detector in sorted(event_detectors):
            ...
            if previous_value is not None and str(previous_value).strip() != "":
                if not minimalwartung:
                    ...
                    continue
                overwritten.append((group, detector, str(previous_value)))
...
def _libreoffice_executable() -> Optional[str]:
    """Findet LibreOffice/soffice unter Windows, macOS und Linux."""
    candidates = [
...
def xls_in_xlsx_konvertieren(xls_path: str | Path, destination_dir: str | Path) -> Path:
    """
    Konvertiert eine alte XLS-Vorlage mit LibreOffice in XLSX.
    ...
    if not source_path.is_file() or source_path.suffix.lower() != ".xls":
        raise Auswertungsfehler("Die gewählte XLS-Prüfliste wurde nicht gefunden.")

    executable = _libreoffice_executable()
    if executable is None:
        raise Auswertungsfehler(
            "Für alte .xls-Prüflisten wird LibreOffice benötigt, damit Formeln erhalten bleiben.\n\n"
            "Bitte LibreOffice installieren und das Programm danach erneut starten."
        )

    expected_output = destination / f"{source_path.stem}.xlsx"
    try:
        ...
        raise Auswertungsfehler(
            f"Die XLS-Datei konnte nicht in XLSX konvertiert werden: {exc}"
        ) from exc

    if process.returncode != 0 or not expected_output.is_file():
        details = (process.stderr or process.stdout or "").strip()
        raise Auswertungsfehler(
            "Die XLS-Datei konnte nicht in XLSX konvertiert werden."
...
def xls_pruefliste_ueber_xlsx_ausfuellen(
    event_path: str | Path,
    xls_path: str | Path,
    output_path: str | Path,
    quarter: int,
    ...
    """Konvertiert XLS nach XLSX und nutzt danach ausschließlich openpyxl."""
    output = Path(output_path)
    if output.suffix.lower() != ".xlsx":
        raise Auswertungsfehler(
            "Eine alte XLS-Vorlage wird aus Sicherheitsgründen immer als .xlsx gespeichert."
        )

    with tempfile.TemporaryDirectory(prefix="BMA_XLS_Konvertierung_") as temp_dir:
        converted = xls_in_xlsx_konvertieren(xls_path, temp_dir)
        return xlsx_pruefliste_ausfuellen(
            ...
            esser_voralarm=esser_voralarm,
            checklist_version=checklist_version,
            minimalwartung=minimalwartung,
        )
...
def pruefliste_ausfuellen(
    ...
    if suffix == ".ods":
        ...
        return ods_pruefliste_ausfuellen(
            event_path=event_path,
            ods_path=checklist_path,
    ...
    if suffix == ".xls":
        if output_suffix != ".xlsx":
            raise Auswertungsfehler(
                "Eine alte XLS-Vorlage wird immer als .xlsx gespeichert, damit Formeln erhalten bleiben."
            )
        return xls_pruefliste_ueber_xlsx_ausfuellen(
            event_path=event_path,
            xls_path=checklist_path,
            output_path=output_path,
            quarter=quarter,
            zeitraum_tage=zeitraum_tage,
            manufacturer=manufacturer,
...
def result_text(result: Dict[str, object]) -> str:
    """Zeigt nach der Auswertung die wichtigen Kontrollwerte an.

    Die Angaben erscheinen ausschließlich nach dem Export, damit nachvollziehbar bleibt, warum
    weniger Zellen neu beschrieben wurden als Melder gefunden wurden.
    """
    found = int(result.get("found", 0))
    inserted = list(result.get("inserted", []))
    overwritten = list(result.get("overwritten", []))
    occupied = list(result.get("occupied", []))
    missing = list(result.get("missing_groups", []))

    lines = [
        "Die Prüfliste wurde erfolgreich verarbeitet.",
        "",
        f"Auswertungszeitraum: "
        f"{result.get('start_date'):%d.%m.%Y} bis {result.get('end_date'):%d.%m.%Y}",
        f"Melder insgesamt erkannt: {found}",
        f"In Prüfliste eingetragen: {len(inserted)}",
        f"Davon bereits belegte Melder überschrieben: {len(overwritten)}",
        f"Bereits belegt und nicht überschrieben: {len(occupied)}",
        f"Meldegruppe nicht in der Prüfliste gefunden: {len(missing)}",
        "",
        "Hinweis: Mehrere FEUER-Meldungen desselben Melders werden nur einmal gewertet.",
        "Bei Esser wird FEUER-ENDE grundsätzlich ignoriert.",
        "VORALARM und VORALARM-ENDE zählen nur, wenn die Voralarm-Option aktiviert ist.",
        "Bereits ausgefüllte Melderfelder werden nur bei aktivierter Minimalwartung überschrieben.",
        "",
    ...
    if result.get("report"):
        lines.extend([
            "",
            f"Prüfbericht gespeichert unter:\n{result['report']}",
        ])
    else:
        lines.extend(["", "TXT-Prüfbericht: nicht erstellt"])

    if missing:
        lines.extend([
            "",
            "Nicht gefundene Gruppen/Melder: "
...
class Application(tk.Tk):
    def __init__(self) -> None:
        ...
        self.minimalwartung_var = tk.BooleanVar(value=False)
    ...
    def build_ui(self) -> None:
        ...
        ttk.Checkbutton(
            form,
            text="Minimalwartung",
            variable=self.minimalwartung_var,
            style="Modern.TCheckbutton",
        ).grid(row=6, column=0, columnspan=3, sticky="w", pady=(4, 4))
        ttk.Label(
            form,
            text="Bereits belegte Melder werden mit dem ausgewählten Quartal überschrieben.",
            style="Panel.TLabel",
            foreground="#aebbc1",
        ).grid(row=7, column=0, columnspan=3, sticky="w", padx=(24, 0), pady=(0, 4))

        ttk.Checkbutton(form, text="TXT-Prüfbericht mit offenen Meldern erstellen", variable=self.create_txt_report_var, style="Modern.TCheckbutton").grid(row=8, column=0, columnspan=3, sticky="w", pady=(4, 10))


        ttk.Button(frame, text="Prüfliste ausfüllen und exportieren", command=self.export, style="Accent.TButton").grid(row=4, column=0, columnspan=3, sticky="ew", pady=(4, 12), ipady=5)

        ttk.Label(frame, textvariable=self.status_var, style="App.TLabel", wraplength=790, justify="left").grid(row=5, column=0, columnspan=2, sticky="w", pady=7)
        action_frame = ttk.Frame(frame, style="App.TFrame")
        action_frame.grid(row=5, column=2, sticky="se", padx=(10, 0), pady=7)
    ...
    def choose_checklist(self) -> None:
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                ...
                ("Alte XLS-Vorlage – Ausgabe wird XLSX", "*.xls"),
                ("Alle Dateien", "*.*"),
            ],
        )
        if path:
            ...
            if Path(path).suffix.lower() == ".xls":
                self.status_var.set("Alte XLS-Vorlage ausgewählt. Die Ausgabe wird automatisch als XLSX gespeichert.")
            else:
                self.status_var.set("Prüfliste ausgewählt. Bereit zur Auswertung.")
    ...
    def export(self) -> None:
        ...
        try:
            ...
            result = pruefliste_ausfuellen(
                ...
                minimalwartung=self.minimalwartung_var.get(),
            )
